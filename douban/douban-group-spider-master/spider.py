...
class DoubanSpider(DBMixin):
    ...
    def fetch(self, url, timeout=10, retury_num=1):
        """发起HTTP请求
    ...
    def run(self):
        ...
        for group_url in self.group_list:
            ...
            greenlet = gevent.spawn(timer.run, self._init_page_tasks, group_url)
    ...
    def _page_loop(self):
        ...
        while 1:
            ...
            gevent.sleep(1)
            self.pool.spawn(self._crawl_page, page_url) #列表页
    ...
    def _topic_loop(self):
        ...
        while 1:
            ...
            self.pool.spawn(self._crawl_detail, topic_url) #详情页
    ...
    def _crawl_page(self, url):
        ...
        @url, str, 当前页面URL
        """
        logger.info("A1 processing page: %s", url)
        html = self.fetch(url)
        ...
        # 保存每页的信息
        self.result_page.insert_many(new_topics)
        # 更新老帖子的时间和回复数
    ...
    def _crawl_detail(self, url):
        ...
        @url, str, 每个帖子的URL
        """
        logger.info("B1，processing topic: %s", url)
        html = self.fetch(url)
        # 获取每一页的信息
        ...
        if self.result_topic.find_one({"url": url}):
            return
        self.result_topic.insert_one(topic)
